"""Run the v2 classifier (DRAFT schema + prompt) over the gold emails — throwaway harness.

LIVE — needs Ollama. Does NOT migrate the pipeline: it calls the LLM client directly
with the v2 contract (EmailExtractionV2) + v2 prompt, replicating the Classifier's
deterministic settings + bounded retry. Predictions are independent of the gold labels,
so this can run while the v2 sheet is still being hand-labeled.

Writes outputs/gold/gold_predictions_v2.csv. Then: python -m app.eval.score_gold_v2

    python -m app.eval.predict_gold_v2
"""
...
from app.agents.prompts_v2 import build_system_prompt, build_user_prompt
from app.config import get_settings
from app.llm.ollama_native import OllamaNativeClient
from app.models.llm_output_v2 import EmailExtractionV2
...
def _rag_candidate(cls) -> bool:
    """Derived in code (never LLM-emitted): the v2 AND-gate."""
    return cls.category == "knowledge_policy_inquiry" and cls.inquiry_answer_source == "kb_policy"
...
def classify_v2(email, client, s):
    """Replicates classifier_extractor.classify(): deterministic call + bounded retry."""
    ...
    for attempt in range(s.classifier_max_retries + 1):
        ...
        r = client.call_structured(system_prompt, user_prompt, response_model=EmailExtractionV2,
...
def predict_one(email_id, client, s) -> dict:
    ...
    out, err = classify_v2(email, client, s)
...
def main() -> None:
    ...
    ids = gold_ids()
    out = OUT_DIR / "gold_predictions_v2.csv"
    ...
    print(f"\n→ {out}\nNow run:  python -m app.eval.score_gold_v2")
